...
router.post("/upload", verifyToken, (req, res) => {
  // Use verifyToken to authenticate
  upload(req, res, async (err) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ success: false, message: "Error uploading to S3" });
    }
  });
});
...
router.get("/files", async (req, res) => {
  ...
  try {
    // Fetch files from MongoDB filtered by courseNumber
    const dbFiles = await File.find(
      courseNumber ? { courseNumber } : {}
    ).lean();
    // Generate signed URLs for each file in MongoDB
      ...
      dbFiles.map(async (file) => {
        const signedUrl = await getSignedUrl(
          ...
          new GetObjectCommand({
            Bucket: process.env.AWS_BUCKET_NAME,
            Key: file.filePath.split(".com/")[1], // Extract the S3 key from the filePath
          }),
          { expiresIn: 3600 }
        ); // 1-hour expiry
        ...
        return {
          ...file,
          filePath: signedUrl, // Replace filePath with signed URL
        };
      })
    );

    res.status(200).json(filesWithSignedUrls); // Send combined data
  } catch (error) {
...
router.get("/courses", async (req, res) => {
  try {
    ...
    const folderNames = data.CommonPrefixes
      ? data.CommonPrefixes.map((prefix) => prefix.Prefix.replace("/", ""))
      : [];
...
router.post("/comments", verifyToken, async (req, res) => {
  ...
  if (!noteId || !content) {
    return res
      .status(400)
      .json({ message: "Note ID and content are required." });
  }
