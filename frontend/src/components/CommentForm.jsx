...
const CommentForm = ({ fileId }) => {
    ...
    return (
        <div className="bg-white p-4 rounded-md shadow-md max-w-2xl mx-auto"> {/* Updated width classes */}
            <form onSubmit={handleCommentSubmit}>
                <label htmlFor="comment" className="sr-only">Comment</label> {/* For better accessibility */}
                <textarea
                    id="comment"
                    value={comment}
                ...
                />
                {error && <p className="text-red-500 text-sm">{error}</p>} {/* Display error message if there's an error */}
                <button
                    type="submit"
